...
class MultipleVersionObjectMixin(object):
    ...
    def get_version_field_name(self):
        """
        Return the model HistoricalRecords field name to use get the history queryset.
        """
        return self.version_field_name if self.version_field_name else 'historico_modificacoes'
    ...
    def get_version_queryset(self):
        ...
        if self.version_queryset is not None:
            ...
            assert hasattr(model_instance, self.get_version_field_name())
            queryset = getattr(model_instance, self.get_version_field_name()).all()
        else:
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not version_allow_empty:
            ...
            # queryset in memory.
            if (self.get_version_paginate_by(self.version_object_list) is not None and hasattr(
                    self.version_object_list, 'exists')):  # noqa
...
class DetailVersionListView(MultipleVersionObjectMixin, generic.DetailView):
    """
    Render some list of django-reversion Version of object, set by `self.model` or
